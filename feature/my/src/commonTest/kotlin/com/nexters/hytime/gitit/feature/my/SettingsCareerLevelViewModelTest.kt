...
/** 개발 수준 선택 화면의 선택·저장 흐름을 검증한다. */
@OptIn(ExperimentalCoroutinesApi::class)
class SettingsCareerLevelViewModelTest {
    ...
    @Test
    fun init_프로필을조회하면_현재수준을선택한다() {
        runTest(dispatcher) {
            val repository = FakeMemberRepository(profileResult = Result.success(PROFILE))
            val viewModel = createViewModel(repository)
            runCurrent()

            assertEquals(CareerLevel.JUNIOR, viewModel.uiState.value.selected)
        }
    }
    ...
    fun backClick_선택이바뀌면_저장하고이전화면으로이동한다() {
        runTest(dispatcher) {
            ...
            viewModel.onCareerLevelClick(CareerLevel.SENIOR)
            viewModel.onBackClick()
            runCurrent()

            assertEquals(CareerLevel.SENIOR, repository.updatedCareerLevel)
            assertEquals(SettingsCareerLevelSideEffect.NavigateBack, sideEffect.await())
        }
    }
    ...
    fun backClick_선택이그대로면_저장하지않고이동한다() {
        runTest(dispatcher) {
            ...
            assertEquals(null, repository.updatedCareerLevel)
            assertEquals(SettingsCareerLevelSideEffect.NavigateBack, sideEffect.await())
        }
    }
    ...
    fun backClick_저장이실패해도_이전화면으로이동한다() {
        runTest(dispatcher) {
            ...
            viewModel.onCareerLevelClick(CareerLevel.MIDDLE)
            viewModel.onBackClick()
            runCurrent()
    ...
    private fun createViewModel(repository: FakeMemberRepository): SettingsCareerLevelViewModel =
        SettingsCareerLevelViewModel(
            getMemberProfile = GetMemberProfileUseCase(repository),
            updateCareerLevel = UpdateCareerLevelUseCase(repository),
        )
